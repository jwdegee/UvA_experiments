...
class staircase_interleaved():
    def __init__(self,subject_initials, task): # self, parameters = {}, phase_durations=[], session=None, screen=None, tracker=None, ID=0,
        ...
        # #----------- The experimenter decides whether or not the participant needs to do one more block.
        # # If set on fullscreen, by pressing Y or N. Else by entering 'yes' or 'no' in the terminal.
        # if fullscreen == True:
        #     yes_no = event.waitKeys(keyList=['y','n'])
        #     for x in yes_no:
        #         if 'n' in x:
        #             one_more_block = 'no'
        #         elif 'y' in x:
        #             one_more_block = 'yes'
        #     print(one_more_block)
        # else:
        #     one_more_block = raw_input('One more block? ')

        # if one_more_block == 'yes':
        #     self.extra_block = visual.TextStim(self.win,pos=[0,+3],text='You will need to do one more block.',wrapWidth=50, color=(-1,-1,-1), font = 'avenir')
        #     self.extra_block2 = visual.TextStim(self.win,pos=[0,-3],text='Press any key to continue',wrapWidth=50, color=(-1,-1,-1), font = 'avenir')
        #     self.extra_block.draw()
        #     self.draw_fixation()
        #     self.extra_block2.draw()
        #     self.win.flip()
        #     event.waitKeys()
        ...
        if task == 'discrim':
            ...
            self.det_perf_all = (float(self.det_low_correct_count)+float(self.det_high_correct_count))/(float(number_of_trials)*4.0)
